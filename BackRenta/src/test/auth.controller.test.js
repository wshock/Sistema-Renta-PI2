...
describe("Controladores de Autenticación - Backend", () => {
  ...
  beforeEach(() => {
    ...
    // Mock del request y response de Express
    mockReq = {
      body: {},
      cookies: {},
      user: {}
  ...
  describe("controlador de registro", () => {
    it("debería registrar un nuevo usuario exitosamente", async () => {
      // Arrange - Configurar los datos de entrada
      mockReq.body = {
        name: "Juan Pérez",
      ...
      // Act - Ejecutar la función del controlador
    ...
    it("debería manejar error de email duplicado (409 Conflicto)", async () => {
      ...
      duplicateError.code = "23505"; // Código de PostgreSQL para violación de unique constraint
      ...
      expect(mockRes.json).toHaveBeenCalledWith(["Este email ya está en uso, prueba con otro."]);
    });
  ...
  describe("controlador de login", () => {
    it("debería hacer login exitosamente con credenciales correctas", async () => {
      ...
      vi.mocked(pool.query).mockResolvedValue({ rows: [mockUser] });
      vi.mocked(bcrypt.compare).mockResolvedValue(true); // Password coincide
      vi.mocked(createAccessToken).mockResolvedValue(mockToken);
    ...
    it("debería retornar 404 para email inexistente", async () => {
      ...
      vi.mocked(pool.query).mockResolvedValue({ rows: [] }); // Usuario no encontrado
    ...
    it("debería retornar 400 para contraseña incorrecta", async () => {
      ...
      vi.mocked(pool.query).mockResolvedValue({ rows: [mockUser] });
      vi.mocked(bcrypt.compare).mockResolvedValue(false); // Password no coincide
